...
function json_request(method, guessString = false) {
    ...
    http.open(method, document.URL + 'l' + '/')
    if (method === "GET") {
        http.onload = () => {
            if (http.status == 200) {
...
function checkGuess() {
    ...
    $.ajax({
        ...
        successFunction(data, guessString)
    });
}

// анализ csv файла
function successFunction(data, guessString) {
    var allRows = data.split(/\r?\n|\r/);
    let ROW = ''
    ROW = allRows[letters - 4]
    if (!ROW.includes(guessString)) {
